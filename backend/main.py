...
# Specify the path to the folder where the CSV file will be stored
csv_folder = os.path.join(script_dir, 'csv_data')

# Make sure the CSV folder exists, create it if it doesn't
os.makedirs(csv_folder, exist_ok=True)

# Specify the name of the CSV file
csv_file_name = f'user_data_{uid}.csv'
...
def write_data_to_csv(statistics):
    file_exists = os.path.isfile(csv_file_path)
    with open(csv_file_path, mode='a', newline='') as csvfile:
        writer = csv.writer(csvfile)
        if not file_exists:
            writer.writerow(['timestamp', 'keystrokes', 'erase_keys', 'erase_keys_percentage',
                             'press_and_hold_avg', 'press_and_hold_std', 'average_latency',
                             'latency_std', 'word_counter', 'word_avg_length', 'word_std_length',
                             'phrase_counter', 'clicks', 'left_clicks', 'right_clicks', 
                             'scroll_clicks', 'scrolls_up', 'scrolls_down', 'avg_speed', 
                             'avg_acceleration', 'directions'])
        for stat in statistics:
...
#             combined_data = [
#                 timestamp, keystroke_counter, erase_counter, erase_counter_percentage,
#                 press_and_hold_avg, press_and_hold_std, average_latency, latency_std, word_counter,
#                 word_avg_length, word_std, phrase_counter + 1, clicks, left_clicks, right_clicks,
#                 scroll_clicks, scrolls_up, scrolls_down, avg_speed, avg_acceleration, str(directions)
#             ]

#             with data_lock:
#                 write_data_to_csv([combined_data])
...
def write_data_to_csv(data, uid):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    csv_folder = os.path.join(script_dir, 'csv_data')
    os.makedirs(csv_folder, exist_ok=True)
    csv_file_name = f'user_data_{uid}.csv'
...
def main(uid):
    ...
    try:
        while True:
            ...
            with data_lock:
                write_data_to_csv(combined_data, uid)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python python_script.py <UID>")
        sys.exit(1)
    uid = sys.argv[1]
    main(uid)
